...
export default function RefillTrackerScreen() {
  ...
  const loadReactives = useCallback(async () => {
    try {
      const allReactives = await getReactives();
  ...
  const handleRefill = async (reactive: Reactive) => {
    try {
      const updatedReactive = {
        ...reactive,
        currentSupply: reactive.totalSupply,
        lastRefillDate: new Date().toISOString(),
      };

      await updateReactive(updatedReactive);
      await loadReactives();

      Alert.alert(
        "Refill Recorded",
        `${reactive.name} has been refilled to ${reactive.totalSupply} units.`
      );
    } catch (error) {
  ...
  const getSupplyStatus = (reactive: Reactive) => {
    const percentage =
      (reactive.currentSupply / reactive.totalSupply) * 100;
    if (percentage <= reactive.refillAt) {
      return {
        status: "Low",
        color: "#F44336",
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        <ScrollView
          style={styles.reactivesContainer}
          showsVerticalScrollIndicator={false}
        >
          {reactives.length === 0 ? (
            <View style={styles.emptyState}>
              <Ionicons name="medical-outline" size={48} color="#ccc" />
              <Text style={styles.emptyStateText}>No reactives to track</Text>
              <TouchableOpacity
                style={styles.addButton}
                onPress={() => router.push("/reactives/add")}
              >
                <Text style={styles.addButtonText}>Add reactive</Text>
              </TouchableOpacity>
            </View>
          ) : (
            reactives.map((reactive) => {
              const supplyStatus = getSupplyStatus(reactive);
              const supplyPercentage =
                (reactive.currentSupply / reactive.totalSupply) * 100;

              return (
                <View key={reactive.id} style={styles.reactiveCard}>
                  <View style={styles.reactiveHeader}>
                    <View
                      style={[
                    ...
                    <View style={styles.reactiveInfo}>
                      <Text style={styles.reactiveName}>
                        {reactive.name}
                      </Text>
                      <Text style={styles.reactiveQuantity}>
                        {reactive.quantity}
                      </Text>
                    </View>
                    <View
                  ...
                  <View style={styles.supplyContainer}>
                    ...
                    <View style={styles.refillInfo}>
                      <Text style={styles.refillLabel}>
                        Refill at: {reactive.refillAt}%
                      </Text>
                      {reactive.lastRefillDate && (
                  ...
                  <TouchableOpacity
                    style={[
                      ...
                      {
                        backgroundColor:
                          supplyPercentage < 100 ? reactive.color : "#e0e0e0",
                      },
                    ]}
                    onPress={() => handleRefill(reactive)}
