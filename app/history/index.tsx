...
export default function HistoryScreen() {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const [doseHistory, reactives] = await Promise.all([
        getDoseHistory(),
        getReactives(),
      ]);

      // Combine history with reactives details
      const enrichedHistory = doseHistory.map((dose) => ({
        ...dose,
        reactive: reactives.find((reac) => reac.id === dose.reactiveId),
      }));
  ...
  const handleClearAllData = () => {
    Alert.alert(
      "Clear All Data",
      "Are you sure you want to clear all reactive data? This action cannot be undone.",
      [
        {
          text: "Cancel",
          style: "cancel",
                ...
                <View key={dose.id} style={styles.historyCard}>
                  <View
                    style={[
                      ...
                      { backgroundColor: dose.reactive?.color || "#ccc" },
                    ]}
                  />
                  <View style={styles.reactiveInfo}>
                    <Text style={styles.reactiveName}>
                      {dose.reactive?.name || "Unknown Reactive"}
                    </Text>
                    <Text style={styles.reactiveQuantity}>
                      {dose.reactive?.quantity}
                    </Text>
